...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		if (R_FAILED(configValid)) {
			char temp[128] = "";
			snprintf(temp, sizeof(temp), "TakesecondsAdvancedGuiListItemText"_tr.c_str(), timeout_in_seconds * sources.size());
    ...
    virtual bool handleInput(u64 keysDown, u64 keysHeld, const HidTouchState &touchPos, HidAnalogStickState joyStickPosLeft, HidAnalogStickState joyStickPosRight) override {
		...
		if (rc == 0x316) {
			strcpy(patchChar, "ConnectionTimeoutAdvancedGuiListItemText"_tr.c_str());
		}
			...
			if (data_to_download != 0) {
				strcpy(patchChar, "ConfigNotAvailable404AdvancedGuiListItemText"_tr.c_str());
				snprintf(progressBar, sizeof(progressBar), "%lu%%", (size_t)(data_downloaded * 100) / data_to_download);
			}
			else strcpy(patchChar, "ConfigNotAvailable404AdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x405) {
			strcpy(patchChar, "ConfigNotAvailableTimeoutAdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x406) {
			strcpy(patchChar, "ConfigNotAvailableConnectionErrorAdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x104) {
			strcpy(patchChar, "NoNewConfigAdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x412) {
			strcpy(patchChar, "InternetConnectionNotAvailableAdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x1001) {
			strcpy(patchChar, "PatchNotNeededAdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x1002) {
			strcpy(patchChar, "NotListedInWarehouseAdvancedGuiListItemText"_tr.c_str());
		}
		else if (rc == 0x1003) {
			...
			strcpy(patchChar, "ListedInWarehousePatchNotAvailableAdvancedGuiListItemText"_tr.c_str());
		}
