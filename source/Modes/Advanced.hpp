...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
		...
		list->addItem(new tsl::elm::NoteHeader("AppliedNextGameBootSetBuffersListItemCategoryHeader"_tr, true, {0xF, 0x3, 0x3, 0xF}));
		...
		if (PluginRunning) {
			if (i > 9) {
				...
				switch (Shared -> API) {
					...
					case 3: {
						if (((Shared -> Buffers) >= 2 && (Shared -> Buffers) <= 4)) {
							sprintf(&nvnBuffers[0], "ActiveBuffersUpdateAdvancedGuiCustomDrawerText"_tr.c_str(), (Shared -> Buffers));
						}
						break;
					}
				}
